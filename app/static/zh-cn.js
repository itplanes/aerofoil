(function () {
    ...
    const exact = new Map(Object.entries({
        ...
        'Base game': '游戏本体', 'Game update': '游戏更新', 'Unknown': '未知',
        'Content identity': '内容身份', 'Content type': '内容类型', 'Base Title ID': '本体 Title ID',
        'Platform compatibility': '平台适用性', 'Not provided by metadata': '元数据未提供',
        'Game family status': '整个游戏族状态', 'This content item': '当前内容项目',
        'Summary for the base game, its known updates, and known DLC.': '汇总游戏本体、已知更新和已知 DLC 的状态。',
        'Only statuses that apply to this selected item are shown here.': '这里只展示明确适用于当前所选项目的状态。',
        'No confirmed library status is available.': '没有可确认的游戏库状态。',
        'Catalog metadata': '目录元数据',
        'Cheat and graphics compatibility': '金手指与画质代码适配',
        'Compatibility requires an exact Build ID match. The catalog version and “Up to date” status are not sufficient.': '适配必须精确匹配 Build ID；目录版本与“已是最新”状态本身不足以判断兼容。',
        'Check compatible codes': '查看可适配代码',
        'Important:': '重要：',
        '“Game Update Current” only compares versions in the configured catalog. It does not confirm whether an update is for Nintendo Switch, Nintendo Switch 2, or both.': '“游戏更新：已是最新”只表示与当前配置目录中的版本相比没有缺失；它不能确认该更新适用于 Nintendo Switch、Nintendo Switch 2，或同时适用于两者。',
